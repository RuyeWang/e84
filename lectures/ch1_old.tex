...
\item {\bf Voltage:} The potential energy per unit charge is measured
  ...
  \[
  v=v_2-v_1=E(l_2-l_1)=El
  \]
  where $l=l_2-l_1$. The electric field can therefore also defined as:
  \[
...
\item {\bf Capacitor}
  ...
  \htmladdnormallink{\em relative static permittivity (dielectric constant)}{https://en.wikipedia.org/wiki/Permittivity}
...
\item {\bf Inductor}
  ...
  \item {\bf Faraday's Law:} 
    ...
    \]
    where $L$ is the {\em inductance} of the coil defined as
    \[ 
    L=\frac{d\Psi}{di}=\frac{d\Psi/dt}{di/dt}=\frac{V}{di/dt}
    \]
    representing the inductor's ability to produce magnetic flux
    $\Psi$ given a varying current, which is determined by the 
    parameters of the inductor: 
    \[
    L=\frac{\mu N^2 A}{l}
    \]
    where $A$ and $l$ are
    respectively the cross section area and length of the coil, $N$ is
    the number of turns, and $\mu$ is the
    \htmladdnormallink{\em magnetic permeability}{https://en.wikipedia.org/wiki/Permeability_(electromagnetism)}
    of the medium inside the coil. When a medium of high permeability, such as an iron core, is inserted into
    the coil, its inductance $L$ is increased.

    Comparing $v(t)=L\dot{i}(t)$ with $v(t)=R\;i(t)$, we see that the 
    inductance $L$ is a resistive variable.

    The unit of $L$ is {\em henrys (H)}
    (\htmladdnormallink{Joseph Henry (1797-1878)}{https://en.wikipedia.org/wiki/Joseph_Henry}):
...
\item {\bf Minimize loss in power transmission line:} 
  ...
  A power transmission line delivers power from power generation (e.g., 
  a power plant) to power consumption (e.g., a city). The concern is no
  ...
  \item Power dissipation along the transmission line $R_T$: 
    \[
    P_T=R_T I^2=R_T \;\left(\frac{P_L}{V_L}\right)^2=R_T \;\frac{P_L^2}{V_L^2}
    \]
  \end{itemize}
