...
\item {\bf Voltage follower:} The input is connected to the positive 
  ...
  \end{equation}
  We see that the output voltage across the load $R_L$ is only a fraction
  of the voltage due to the voltage drop $v_s R_s/(R_L+R_s)$ across the 
  internal resistance $R_s$. If it is desired for the output voltage to 
  be as close to the source as possible, the internal resistance $R_s$ 
  ...
  modeled by its input and output resistances $R_{in}$ and $R_{out}$, as
  well as its voltage gain $G_{oc}$, as shown in the right figure. The 
  ...
  \begin{equation} 
    ...
    \approx v_s
  \end{equation}
  We see that the output voltage across $R_L$ can be very close to 
  source voltage, i.e.,$v_{out}\approx v_s$, due to the nature of the
  op-amp:
  \begin{itemize}
  \item $G_{oc} \approx 1$
  \item 
