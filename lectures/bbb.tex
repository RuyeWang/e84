...
component out and treat it as the load $R_L$ of the rest of the circuit 
treated as a voltage source $(V_T, R_T)$, and then find the $V$ and $I$ at 
its output port.
...
network be $V$ and $I$, respectively. 
\begin{itemize}
\item Replace the load $R_L$ by an ideal current source $I=V/R_L$ while 
  keeping voltage $V$ the same (b), the voltage or current anywhere in the 
...
\item Find the terminal voltage $V$ in terms of the internal energy sources 
  ...
  principle:
  \begin{itemize}
  \item When the external current source is turned off (open-circuit), the 
    terminal voltage is the open-circuit voltage $V'=V_{oc}$ due only to the
    sources internal to the network (c).
  \item When all internal sources are turned off (short-circuit for voltage
    sources, open-circuit for current sources), the terminal voltage is
    $V''=-R_0I$, where $R_0$ is the equivalent resistance of the network with
    all energy sources off (d).
  \end{itemize}
  The overall terminal voltage is $V=V'+V''=V_{oc}-R_0I$. 
\end{itemize}
We see that as far as the port voltage $V$ and current $I$ are concerned, 
the one-port network is equivalent to an ideal voltage source $V_T=V_{oc}$ 
equal to the open-circuit voltage across the port, in series with an internal 
resistance $R_T=R_0$, which can be obtained as the ratio of the open-circuit 
...
the resistance along the branch. Similarly, the voltage source provides a
constant voltage $V_0$ to the rest of the circuit, independent of any 
