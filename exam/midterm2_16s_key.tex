...
\item Due Monday (4/4) in class.
\item Mark your name clearly on top of each page. Indicate the total number of
  pages submitted.
...
A source voltage $v(t)=2\cos (314 t)\;V$ is applied to the bridge circuit
below, with $L=0.1\,mH$ and $C=1\,\mu F$. It is known that the voltage at
point a remains the same no matter whether the switch is open or closed.
%i.e., the bridge circuit is balanced.
\begin{enumerate}
\item Find $R$
\item Find total impedance $Z$ between point a and ground.
\item Find $v_a(t)$ at point a.
\end{enumerate}

\htmladdimg{../midterm2x.png}

{\bf Solution:}

The bridge is balanced, i.e, 
\[
\frac{R}{1/j\omega C}=\frac{j\omega L}{R},\;\;\;\;\;\;R^2=L/C=10^2
\]
$R=10\,\Omega$. The total impedance of the bridge
\[
Z=\frac{(R+1/j\omega C)(R+j\omega L)}{R+1/j\omega C)+(R+j\omega L)}
  =\frac{(R^2+L/C+R(j\omega L+1/j\omega C)}{2R+(j\omega L+1/j\omega C)}
  =R
\]
$v_a(t)=\cos(314 t)$
...
$R$ and the input sinusoidal voltage $V$ in frequency domain. 
